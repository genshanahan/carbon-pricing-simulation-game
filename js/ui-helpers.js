...
export function outputBudgetAnalogy(output, budgetUsed) {
  ...
  return `<strong>Total Economic Output</strong> measures the total economic value created by the industry \u2014 total revenue minus total costs (production costs plus any clean-tech investment). It is equivalent to firm profit plus tax revenue, but only because the tax terms cancel out: what firms pay in tax, the government receives, so tax redistributes value within the economy without creating or destroying it. The differences in TEO between regimes are driven entirely by how many units were produced and whether clean-tech investment costs were incurred. <strong>Carbon Budget Used</strong> shows ppm added as a percentage of the safe carbon budget (the headroom between the starting ppm and the catastrophe trigger). A figure above 100% means the regime overshot the safe threshold. The two figures are shown side by side rather than collapsed into a single ratio so that the trade-off between output and climate impact remains visible. Neither figure alone tells the whole story: what was produced, for whom, and at what climate cost are all separate questions worth discussing.`;
}
  ...
  const notes = {
    ...
    cac: {
      ...
      debriefTips: [
        ...
        'Ask: "What do the Total Economic Output and Carbon Budget Used figures tell us?" (TEO is total revenue minus total costs \u2014 the economic value the industry created. Tax washes out: it reduces firm profit but adds equally to government revenue, so it neither creates nor destroys value. The only drivers of TEO differences across regimes are total production volume and clean-tech investment costs. Budget Used shows how much of the safe carbon headroom was consumed; values above 100% mean the regime overshot. Together they let students compare regimes on output and climate impact without collapsing the trade-off into one number, and without implying that more output is always desirable \u2014 that depends on what is being produced and for whom.)',
 ...
 * Facilitator hints are shown only when showHints=true (host-app, solo-app).
 */
export function renderDiscussionCard(config, showHints = false) {
  const hint = (text) => showHints
    ? `<details style="margin:0.15rem 0 0.6rem 0.5rem;font-size:0.82rem;"><summary style="cursor:pointer;color:var(--text-secondary);font-style:italic;">Facilitator hint</summary><div style="margin-top:0.3rem;color:var(--text-secondary);padding-left:0.5rem;border-left:2px solid #ddd;">${text}</div></details>`
    : '';
  return `
    <div class="card">
      <h2>Discussion</h2>
      <div class="debrief-box" style="background:#eaf2f8;border-color:#aed6f1;">
        ...
        <ol>
          <li>Which regime actually kept emissions within the safe threshold \u2014 and at what cost to economic output?</li>
          ${hint('Use the Output/Budget figures from the regime summaries to anchor this concretely. The key tension: C&C may protect the atmosphere but wastes productive capacity (deadweight loss); cap-and-trade may optimise output but only under conditions the game guarantees (honest caps, no lobbying, perfect enforcement). Push students to notice that \u201cefficiency\u201d is only meaningful relative to a constraint that holds.')}
          <li>The game gave you perfect information: known costs, known emissions per unit, a fixed and certain catastrophe threshold. What happens to each regime\u2019s effectiveness when those certainties break down \u2014 when the safe threshold turns out to be lower than assumed, when costs shift unpredictably, or when the science changes mid-policy?</li>
          ${hint('This is the Weitzman price-vs-quantity problem. A carbon tax gives certainty about the <em>price</em> of emissions but not the <em>quantity</em> \u2014 so if the science says we\u2019re closer to a tipping point than we thought, the tax doesn\u2019t automatically adjust. A cap gives certainty about the <em>quantity</em> but not the <em>price</em> \u2014 so firms face unpredictable costs. Ask students: which kind of uncertainty is more dangerous for the climate? Which is more dangerous for firms? Those are different questions with different answers, and who gets to decide between them is a political question, not a technical one.')}
          <li>The game offered market-based flexibility as the route to efficiency, but it never modelled the option of simply banning high-emission production. Are there cases where rigid rules \u2014 removing options rather than pricing them \u2014 would deliver better environmental outcomes than price signals?</li>
          ${hint('The game is calibrated to make C&C look wasteful relative to pricing \u2014 that\u2019s the orthodox economic story, and students should now be able to articulate it clearly. The critical counter: removing high-emission options from the market entirely (e.g. banning internal combustion engines by a target date, or phasing out coal without a price mechanism) avoids the gaming, monitoring, and enforcement costs that plague pricing mechanisms. It also sidesteps the information problem in Q2: a ban doesn\u2019t require knowing the \u201cright\u201d price or the \u201cright\u201d cap \u2014 it simply eliminates the harmful activity. Ask students to think of real-world examples where prohibition has been more effective than pricing (e.g. CFCs under the Montreal Protocol, lead in petrol).')}
        </ol>
      </div>
      <div class="debrief-box" style="background:#fef5e7;border-color:#f9e2b0;">
        <h3 style="color:#e67e22;">Normative Desirability</h3>
        <ol>
          <li>The game sets catastrophe at ${config.triggerPpm} ppm, corresponding to the Paris Agreement\u2019s 2\u00b0C ceiling. But the CO\u2082 meter shows that serious human harm \u2014 mass displacement, lethal heat, food system stress \u2014 begins well before that line. What human costs and moral trade-offs are baked into choosing <em>this</em> threshold rather than a lower one? And who decided?</li>
          ${hint('The game\u2019s PPM bands make this visible: by 450 ppm (+1.5\u00b0C), heat-related mortality has roughly doubled and tens of millions more face food insecurity. The ' + config.triggerPpm + ' ppm \u201ccatastrophe\u201d threshold implicitly accepts those costs as tolerable \u2014 push students to ask: tolerable <em>for whom</em>? The Paris 2\u00b0C ceiling was a political compromise, not a scientific determination that 2\u00b0C is \u201csafe.\u201d The people most exposed to the harm between 1.5\u00b0C and 2\u00b0C \u2014 those in climate-vulnerable regions, low-income communities, future generations \u2014 were least represented in that negotiation. The threshold itself is a normative choice disguised as a technical parameter, and it is worth surfacing that the game inherits that choice uncritically.')}
          <li>The game represents only competing firms \u2014 not workers, consumers, or communities affected by production or by the climate impacts the PPM meter tracks. How would including those perspectives change which regime looks \u201cfairest\u201d?</li>
          ${hint('This question asks who is structurally absent from the game\u2019s model: workers, consumers, and communities who bear the health and environmental costs of production decisions they had no part in making. Push students to name specifically what those perspectives would add, not just acknowledge abstractly that the game is incomplete.')}
          <li>Under the carbon tax, firms that could not afford clean technology bore a disproportionate cost. A tonne of CO\u2082 is taxed the same regardless of the firm\u2019s capacity to abate. Is that fair?</li>
          ${hint('This question surfaces the regressive distributional logic of uniform pricing. The game makes this visible through the clean-tech toggle: firms that invested paid half the effective tax rate per unit. Push students to generalise: in the real world, a carbon price treats all tonnes as equivalent regardless of whether they come from a hospital heating system or a private jet. The economic case for carbon pricing is an efficiency argument \u2014 but efficiency and fairness are different criteria, and a policy can score well on one while performing poorly on the other.')}
          <li>A carbon price works by converting plural values \u2014 health, justice, ecological integrity, intergenerational responsibility \u2014 into a single number. The game made that commensuration feel natural: everything reduced to dollars and PPM. What is gained by that coordinative power \u2014 and what is lost?</li>
          ${hint('Commensuration \u2014 converting diverse values into a single comparable metric \u2014 is genuinely powerful. It allows millions of actors with different knowledge and priorities to coordinate without needing to agree on everything; a price signal can guide decisions more accurately than crude heuristics like \u201cbuy local.\u201d But metrics also shape the behaviour they measure: once a number becomes a target, actors orient toward improving the metric rather than the underlying reality it was meant to track (Goodhart\u2019s Law). In carbon markets this has meant lobbying over cap levels, offset verification failures, and speculative trading detached from physical emissions. Ask students: what does the game count, and what does it leave out \u2014 and what would a decision-maker miss if they relied only on the numbers the game produces?')}
          <li>The game presents regulation as a restriction on freedom. But the Free Market regime showed that \u201cfreedom\u201d produced collective self-destruction. Is the most important function of regulation to allow flexibility \u2014 or to remove destructive options entirely? Can constraints be <em>enabling</em>?</li>
          ${hint('The cap didn\u2019t just restrict firms \u2014 it <em>freed</em> them from the prisoner\u2019s dilemma. Without it, each firm\u2019s rational individual choice (maximise production) produced a collectively irrational outcome (catastrophe). This reframes the standard efficiency critique of command-and-control: the question is not only which mechanism is cheapest, but what kind of freedom regulation is for. Some argue that the most consequential climate policies are not prices but prohibitions \u2014 bans on new fossil fuel extraction, mandatory building standards, product regulations that remove high-emission options from the market altogether. Ask students: is that a loss of freedom, or a precondition for collective action? And who benefits from framing regulation primarily as a constraint on freedom rather than as an enabler of coordination?')}
        </ol>
      </div>
      <div class="debrief-box" style="background:#fdf2f2;border-color:#f5c6cb;">
        <h3 style="color:#c0392b;">Political Feasibility</h3>
        <ol>
          <li>In the game, the regulator set the cap and the tax rate and they held. In the real world, caps are set by politicians under pressure from industry. The EU ETS saw carbon prices crash repeatedly because states issued too many permits. Is carbon pricing vulnerable to capture precisely because it is \u201cregulation lite\u201d \u2014 politically acceptable because it doesn\u2019t fundamentally threaten powerful interests?</li>
          ${hint('Emissions trading tends to be politically attractive because it is non-threatening to industry \u2014 firms receive permits, retain flexibility, and face no outright prohibition on their activities. But that acceptability may come at the cost of environmental integrity: the \u201ccap\u201d that is supposed to guarantee emissions reductions is set by the same political process that is subject to industry lobbying. The EU ETS is the most-studied example: carbon prices remained too low for over a decade to drive real investment in abatement. Push students to consider whether the game\u2019s clean \u201ccap holds, firms adjust\u201d logic could survive contact with real political institutions \u2014 and whether a mechanism that is politically sustainable is necessarily one that works.')}
          <li>The game models carbon pricing as a direct, transparent mechanism: emit, pay, adjust. In the real world, carbon markets have tended toward financialisation \u2014 second-order trading, speculation, and offsets of questionable quality. Is that a corruption of the mechanism, or is it what market-based regulation inevitably produces when financial actors enter the picture?</li>
          ${hint('The game\u2019s bilateral permit trades gave students a taste of price discovery through direct negotiation. In practice, permit markets develop derivatives, futures, and speculative instruments that decouple trading activity from physical emissions. Carbon offsets extend the problem further: investigative journalism has found that the majority of offsets sold by major certifiers do not represent genuine emissions reductions. Ask students to consider whether this is a fixable implementation problem \u2014 better standards, stricter verification, stronger enforcement \u2014 or a structural tendency of market-based instruments that abstract a physical constraint (the atmosphere\u2019s capacity) into a tradeable financial asset.')}
          <li>Imagine you are a government minister. You have just played this game and you are persuaded that cap-and-trade is theoretically optimal. Now: you need to pass it through parliament, enforce it across industries, coordinate it internationally, and sustain it across election cycles. What political conditions would need to hold \u2014 and how realistic are they?</li>
          ${hint('This question asks students to move from the game\u2019s frictionless institutional setting to real-world implementation. A useful distinction here is between short-term achievability (can you pass the legislation?) and longer-term political viability (can you sustain it through changing governments, industry resistance, and shifting public priorities?). Carbon pricing requires durable cross-party commitment, robust enforcement institutions, and ideally international coordination to prevent carbon leakage \u2014 none of which the game models. Push toward concrete examples: Australia repealed its carbon tax after one election cycle; the EU ETS has required constant structural reform over two decades. What does this tell us about the political feasibility of market-based climate policy as a primary decarbonisation strategy \u2014 as opposed to one tool among many?')}
        </ol>
      </div>
    </div>`;
}
